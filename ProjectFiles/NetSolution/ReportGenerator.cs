...
public class ReportGenerator : BaseNetLogic
{
    ...
    public override void Start()
    {
        ...
        TotalCustomSettings = LogicObject.GetVariable("TotalCustomSettings");
        TotalGlueSettings = LogicObject.GetVariable("TotalGlueSettings");
        //GlueOption = LogicObject.GetVariable("GlueOption");
    }
    ...
    private void AddFirstPage(Document document, PdfDocument pdf, string machinePath, PdfFont fontBold, string title)
    {
        ...
        logo.SetFixedPosition((pageSize.GetWidth() - 600) / 2, titleY - 425);  // Center image horizontally, adjust Y position to fit below title
        logo.ScaleToFit(600, 500);  // Scale the image
    ...
    private List<Dictionary<string, object>> GetTransposedRows()
    {
        ...
        string query = $"SELECT {string.Join(", ", selectedColumns)} FROM COAndGlueSchema ORDER BY Name";
        Log.Info($"Generated Query: {query}");

        var myStore = Project.Current.Get<Store>("Data/DataStores/COAndGlueRecipeDB");
        Object[,] ResultSet;
        String[] Header;
        ...
        var headerRow = new Dictionary<string, object>
    {
        ...
        //TotalCustomSettings = LogicObject.GetVariable("TotalCustomSettings");
        var totalCustomOptions = TotalCustomSettings.Value;

        //var glueMachine = LogicObject.Owner.GetObject("COReportVariables").GetVariable("GlueOption");

        if (totalCustomOptions > 0)
        {
            var customRows = GetTransposedCustomRows();
            transposedRows.AddRange(customRows); // Append glue rows to the end
        }

        var glueRows = GetTransposedGlueRows();
        transposedRows.AddRange(glueRows); // Append glue rows to the end
    ...
    private List<Dictionary<string, object>> GetTransposedGlueRows()
    {
        var totalGlueSettings = TotalGlueSettings.Value;


        // Get glue descriptions from the method
        var glueDescriptionsDict = getGlueDescriptions();

        // Query for glue settings
        List<string> selectedColumns = new List<string> { "Name" };
        for (int i = 1; i <= totalGlueSettings; i++)
        {
            selectedColumns.Add($"\"/GlueSettings_{i}\"");
        }

        string query = $"SELECT {string.Join(", ", selectedColumns)} FROM COAndGlueSchema ORDER BY Name";
        Log.Info($"Generated Query for Glue Settings: {query}");
        ...
        var glueRows = new List<Dictionary<string, object>>();
        for (int i = 1; i <= totalGlueSettings; i++)
        {
            var glueRow = new Dictionary<string, object>
        {
            // Add the description for the glue setting in the "Adjustment Description" column
            { "Adjustment Description", glueDescriptionsDict.ContainsKey(i)
                ? $"Glue Setting {i}: {glueDescriptionsDict[i]}"
                : $"Glue Setting {i}" }
        };

            for (int row = 0; row < ResultSet.GetLength(0); row++)
            {
                string glueSettingName = ResultSet[row, 0]?.ToString();
                int glueColumnIndex = i;
                if (glueColumnIndex < ResultSet.GetLength(1))
                {
                    glueRow[glueSettingName] = ResultSet[row, glueColumnIndex];
                }
            }

            if (glueRow.Values.Any(value => value != null && value.ToString() != "0"))
            {
                glueRows.Add(glueRow);
            }
        }
    ...
    private List<Dictionary<string, object>> GetTransposedCustomRows()
    {
        var totalCustomSettings = TotalCustomSettings.Value;


        // Get custom descriptions from the method
        var customDescriptionsDict = getCustomDescriptions();

        // Query for custom settings
        List<string> selectedColumns = new List<string> { "Name" };
        for (int i = 1; i <= totalCustomSettings; i++)
        {
            selectedColumns.Add($"\"/CustomOptionsSettings_{i}\"");
        }

        string query = $"SELECT {string.Join(", ", selectedColumns)} FROM COAndGlueSchema ORDER BY Name";
        Log.Info($"Generated Query for custom Settings: {query}");
        ...
        var customRows = new List<Dictionary<string, object>>();
        for (int i = 1; i <= totalCustomSettings; i++)
        {
            var customRow = new Dictionary<string, object>
        {
            // Add the description for the custom setting in the "Adjustment Description" column
            { "Adjustment Description", customDescriptionsDict.ContainsKey(i)
                ? $"Option {i}: {customDescriptionsDict[i]}"
                : $"Option {i}" }
        };

            for (int row = 0; row < ResultSet.GetLength(0); row++)
            {
                string customSettingName = ResultSet[row, 0]?.ToString();
                int customColumnIndex = i;
                if (customColumnIndex < ResultSet.GetLength(1))
                {
                    customRow[customSettingName] = ResultSet[row, customColumnIndex];
                }
            }

            if (customRow.Values.Any(value => value != null && value.ToString() != "0"))
            {
    ...
    private Dictionary<int, string> getCustomDescriptions()
    {
        var customDescriptions = Project.Current.GetObject("Model/Descriptions/CustomOptionsDescriptions");
        var customDescriptionsChildren = customDescriptions.Children;
        var customDescriptionsDict = new Dictionary<int, string>();

        foreach (var customDesc in customDescriptionsChildren)
        {
            var customDescriptionVariable = customDesc as UAVariable;
            if (customDescriptionVariable == null)
            {
            ...
            string browseName = customDesc.BrowseName;
            if (int.TryParse(new string(browseName.Where(char.IsDigit).ToArray()), out int index))
            {
                LocalizedText customDescription = (LocalizedText)customDescriptionVariable.Value;
                if (customDescription != null)
                {
                    customDescriptionsDict[index] = customDescription.TextId;
                    if (customDescription.TextId == "")
                    {
                        customDescriptionsDict[index] = customDescription.Text;
                    }
                    else
                    {
                        customDescriptionsDict[index] = customDescription.TextId;
        ...
        return customDescriptionsDict;
    }
    private Dictionary<int, string> getGlueDescriptions()
    {
        var glueDescriptions = Project.Current.GetObject("Model/Descriptions/GlueDescriptions");
        var glueDescriptionsChildren = glueDescriptions.Children;
        var glueDescriptionsDict = new Dictionary<int, string>();

        foreach (var glueDesc in glueDescriptionsChildren)
        {
            var glueDescriptionVariable = glueDesc as UAVariable;
            if (glueDescriptionVariable == null)
            {
                Log.Error("Child is not a UAVariable");
                continue;
            }
            ...
            // Extract the integer index from the BrowseName
            string browseName = glueDesc.BrowseName;
            if (int.TryParse(new string(browseName.Where(char.IsDigit).ToArray()), out int index))
            {
                LocalizedText glueDescription = (LocalizedText)glueDescriptionVariable.Value;
                if (glueDescription != null)
                {
                    glueDescriptionsDict[index] = glueDescription.TextId;
                    if (glueDescription.TextId == "")
                    {
                        glueDescriptionsDict[index] = glueDescription.Text;
                    }
                    else
                    {
                        glueDescriptionsDict[index] = glueDescription.TextId;
