...
public class SelectFileDialogLogic : BaseNetLogic
{
    ...
    private SelectFileDialogEntry CreateFilesystemEntry(FileSystemInfo entry, bool isDirectory)
    {
        ...
        if (!isDirectory)
        {
            ...
            var tmpFile = ResourceUri.FromAbsoluteFilePath(entry.FullName);
            fileSystemEntry.FileUri = tmpFile;
        }
        else
        {
            var tmpFile = ResourceUri.FromProjectRelativePath("FilesystemBrowser\\Folder_normal.svg");
            fileSystemEntry.FileUri = tmpFile;
        }
